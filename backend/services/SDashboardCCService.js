...
async function GiveCCDashboard(RegisterNumber, password) {
    try {
        ...
        // Find all courses enrolled by the student with attendance status true
        const courses = await Course.find({ 
            courseCode: { $in: student.CourseEnrolled },
        ...
        const studentDetails = {
            ...
            courses: await Promise.all(courses.map(async course => {
                ...
                const presentStudents = course.presentStudents || [];
                courseDetails.presentStatus = presentStudents.includes(RegisterNumber);
...
module.exports = { GiveCCDashboard };
