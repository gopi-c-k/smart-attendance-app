const FloginService = require('../services/FloginService');

exports.Flogin = async (req, res, next) => {
    try {
        ...
        const loginResult = await FloginService.FloginUser(UserID, Password);

        // Check the login result
        if (loginResult.success) {
            // Login successful, return the student details
            res.status(200).json({ success: true, message: 'Login successful', faculty : loginResult.faculty });
        } else {
