const { GetCoursesTaughtByFaculty } = require('../services/FDashboardService');

exports.GetCoursesTaughtByFacultyController = async (req, res, next) => {
    try {
        const registerID = req.params.regNo;
        const password = req.body.password;
        
        // Call the GetCoursesTaughtByFaculty method from the service
        const result = await GetCoursesTaughtByFaculty(registerID, password);
